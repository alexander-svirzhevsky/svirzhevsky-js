...
tryToLoad()
  ...
  .catch(() => loadingBlock.classList.add('hidden'))
  .catch(() => loadingFailedBlock.classList.remove('hidden'));
...
function updFilter(filterValue) {
  ...
  for (const iterator of towns) {
    if (filterValue && isMatching(iterator.name, filterValue)) {
      const townDiv = document.createElement('div');
      townDiv.textContent = iterator.name;
